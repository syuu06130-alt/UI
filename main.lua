...
local Theme = {
    ...
    BGSec           = Color3.fromRGB(16,  16,  24),
    BGTer           = Color3.fromRGB(22,  22,  34),
    BGHover         = Color3.fromRGB(28,  28,  44),

    -- アクセント (Electric Purple)
    Accent          = Color3.fromRGB(124, 92, 255),
    AccentDark      = Color3.fromRGB(90,  60, 210),
    AccentLight     = Color3.fromRGB(165, 133, 255),
    AccentGlow      = Color3.fromRGB(124, 92, 255),

    -- 2nd アクセント (Cyan highlight)
    Cyan            = Color3.fromRGB(80,  220, 255),
    CyanDark        = Color3.fromRGB(40,  160, 200),

    -- テキスト
    TextPri         = Color3.fromRGB(240, 238, 255),
    TextSec         = Color3.fromRGB(155, 152, 185),
    TextMut         = Color3.fromRGB(75,  72,  105),

    -- ボーダー
    Border          = Color3.fromRGB(40,  38,  65),
    BorderBright    = Color3.fromRGB(75,  72,  120),

    -- セマンティック
    Success         = Color3.fromRGB(50,  215, 130),
    Warning         = Color3.fromRGB(255, 190, 60),
    Error           = Color3.fromRGB(240, 75,  85),
    Info            = Color3.fromRGB(80,  175, 255),

    -- Toggle
    TogOn           = Color3.fromRGB(124, 92, 255),
    TogOff          = Color3.fromRGB(38,  36,  60),
    TogKnob         = Color3.fromRGB(255, 255, 255),

    -- Pill
    PillBG          = Color3.fromRGB(16,  16,  24),
    PillBorder      = Color3.fromRGB(124, 92, 255),
}
...
local Cfg = {
    ...
    QuartOut        = TweenInfo.new(0.22, Enum.EasingStyle.Quart, Enum.EasingDirection.Out),
...
-- グロウパルスアニメーション
function Util.PulseGlow(frame, col)
...
function SyuHub:CreateWindow(opts)
    ...
    local Pill = Util.New("Frame", {
        ...
        Position = UDim2.new(0.5, -(Cfg.PillW / 2), 0, 8),
    ...
    Util.Corner(Pill, UDim.new(0, Cfg.PillH / 2))
    Util.Stroke(Pill, Theme.PillBorder, 1, 0)
    Util.PulseGlow(Pill, Theme.Accent)
    ...
    -- 環境光（上部グロウ）
    local ambientGlow = Util.New("Frame", {
        Name = "AmbientGlow",
    ...
    Util.Corner(ambientGlow, UDim.new(0, 14))
    ...
    local bgc = Main:FindFirstChild("BGCircle")
    if bgc then Util.Corner(bgc, UDim.new(0, 80)) end
    ...
    Util.Corner(accentLine, UDim.new(0, 1))

    -- ロゴ
    local LogoBg = Util.New("Frame", {
        Name = "LogoBG",
    ...
    Util.Corner(LogoBg, UDim.new(0, 10))
    ...
    Util.Corner(logoGlow, UDim.new(0, 15))
    ...
    Util.List(HBtns, Enum.FillDirection.Horizontal, 6)

    local function MakeHBtn(text, size)
        local btn = Util.New("TextButton", {
            Text = text,
            Size = UDim2.new(0, size or 32, 0, 32),
            BackgroundColor3 = Theme.BGTer,
            BorderSizePixel = 0,
            TextColor3 = Theme.TextSec,
            TextSize = size and 11 or 13,
            Font = Cfg.Font,
            ZIndex = 13,
            ClipsDescendants = true,
            Parent = HBtns,
        })
        Util.Corner(btn, UDim.new(0, 9))
        btn.MouseEnter:Connect(function() Util.Tween(btn, { BackgroundColor3 = Theme.BGHover }) end)
        btn.MouseLeave:Connect(function() Util.Tween(btn, { BackgroundColor3 = Theme.BGTer  }) end)
        btn.MouseButton1Down:Connect(function(x, y) Util.Ripple(btn, x, y) end)
        return btn
    end

    local SettingsBtn  = MakeHBtn("⚙", 32)
    local MinimizeBtn  = MakeHBtn("—", 32)
    ...
    local DragPill = Util.New("Frame", {
        ...
        Position = UDim2.new(0.5, -18, 0.5, -2),
    ...
    local function Minimize()
        ...
        Main.Visible = false
        Pill.Visible = true
        Pill.Size = UDim2.new(0, 0, 0, Cfg.PillH)
        Util.Spring(Pill, { Size = UDim2.new(0, Cfg.PillW, 0, Cfg.PillH) }, Cfg.AnimSpd)
    end

    local function Expand()
        minimized = false
        Util.Tween(Pill, { Size = UDim2.new(0, 0, 0, Cfg.PillH) },
            TweenInfo.new(Cfg.AnimSpd - 0.06, Enum.EasingStyle.Back, Enum.EasingDirection.In))
        task.wait(Cfg.AnimSpd - 0.04)
        Pill.Visible = false
        Main.Visible = true
    ...
    Pill.MouseLeave:Connect(function()
        Util.Tween(Pill, { BackgroundColor3 = Theme.PillBG })
    ...
    function Window:CreateTab(name, icon)
        ...
        Util.Corner(accent, UDim.new(0, 2))
...
local Syu = loadstring(game:HttpGet("your_raw_url"))()
